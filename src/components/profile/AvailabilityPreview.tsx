...
export default function AvailabilityPreview({ mentorId, onSeeMore, mentor }: AvailabilityPreviewProps) {
  ...
  const generateCalendarDays = (availabilityMap: Map<string, number>, blockedDates: Set<string>) => {
        ...
        {loading ? (
          ...
          <>
                ...
                {calendarDays.map((day, index) => {
                  ...
                  return (
                    <div
                      ...
                      title={`${day.date.toLocaleDateString()} - ${day.status} ${day.timeslotCount > 0 ? `(${day.timeslotCount} slots)` : ''}`}
                    >
                      ...
                      <div className={`w-1.5 h-1.5 rounded-full ${getDotColor(day.status)}`} />
                    </div>
                  );
                })}
              </div>
            ...
            {availableDates.length > 0 && (
              <div className="mb-4">
                <h4 className="font-semibold text-sm text-gray-900 mb-3">Next Available Dates</h4>
