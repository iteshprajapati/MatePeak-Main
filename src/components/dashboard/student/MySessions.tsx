...
export default function MySessions({ studentProfile }: MySessionsProps) {
  ...
  const filterSessions = () => {
    ...
    switch (activeTab) {
      case 'upcoming':
        filtered = filtered.filter(s => 
          new Date(s.session_date) > now && s.status !== 'cancelled'
        );
        break;
      case 'past':
        filtered = filtered.filter(s => 
          new Date(s.session_date) <= now || s.status === 'completed'
        );
        break;
      case 'cancelled':
    ...
    if (searchQuery) {
      filtered = filtered.filter(s =>
        s.expert_profiles?.full_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const exportToCSV = () => {
    const csv = [
      ['Date', 'Time', 'Mentor', 'Expertise', 'Duration', 'Status', 'Price'].join(','),
        ...
        <TabsList className="grid w-full grid-cols-4">
          <TabsTrigger value="upcoming">
            Upcoming ({sessions.filter(s => 
              new Date(s.session_date) > new Date() && s.status !== 'cancelled'
            ).length})
          </TabsTrigger>
          <TabsTrigger value="past">
            Past ({sessions.filter(s => 
              new Date(s.session_date) <= new Date() || s.status === 'completed'
            ).length})
          </TabsTrigger>
                      ...
                      <div className="flex-1 space-y-2">
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {session.expert_profiles?.expertise?.join(', ') || 'Expert'}
                            </p>
                          </div>
                        ...
                        <div className="flex flex-wrap gap-4 text-sm text-gray-700">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span>{formatTime(session.session_date)} • {session.duration || 60} min</span>
                          </div>
                          {session.total_price && (
                        ...
                        <div className="flex flex-wrap gap-2 pt-2">
                          {session.status === 'confirmed' && new Date(session.session_date) > new Date() && (
                            <>
                          ...
                          {session.status === 'cancelled' && new Date(session.session_date) > new Date() && (
                            <Button size="sm" variant="outline" className="flex items-center gap-1">
          ...
          <CardContent className="p-4">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-gray-900">
                Total Spent: ${filteredSessions.reduce((sum, s) => sum + (s.total_price || 0), 0).toFixed(2)}
              </span>
            </div>
          </CardContent>
        </Card>
