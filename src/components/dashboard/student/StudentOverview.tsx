...
export default function StudentOverview({ studentProfile, onNavigate }: StudentOverviewProps) {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const now = new Date();
      const upcoming = bookingsList.filter(b => new Date(b.session_date) > now && b.status !== 'cancelled');
      const completed = bookingsList.filter(b => b.status === 'completed');

      // Calculate stats
      const uniqueMentors = new Set(bookingsList.map(b => b.expert_id)).size;
      const totalMinutes = completed.reduce((sum, b) => sum + (b.duration || 60), 0);
      ...
      const pendingReviews = completed.filter(b => !reviewedBookingIds.has(b.id)).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-baseline mt-3">
                  <p className="text-3xl font-bold text-gray-900">
                    {stats.upcomingSessions + Math.floor(stats.totalHours * 1.5)}
                  </p>
                </div>
              </div>
        ...
        {/* Earnings Card - Coming Soon */}
        <Card className="group bg-gray-100 border-0 rounded-2xl shadow-none hover:shadow-md transition-all duration-200 relative overflow-hidden">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
        ...
        <CardContent className="p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            ...
            >
              ...
              <span className="text-sm font-medium text-gray-900">Browse Mentors</span>
            </button>
                ...
                <div className="flex-1">
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    <div className="flex items-center gap-2 text-gray-700">
                      ...
                      <span>{formatTime(nextSession.session_date)} • {nextSession.duration || 60} minutes</span>
                    </div>
                  </div>
                </div>
              </div>
